...
export default function ViewerPage({ params }: ViewerPageProps) {
  const { BookID } = params;
  const { bookContent, isLoading, bookTitle, bookChapters } = useBookData(BookID); // ✅ API에서 책 데이터 가져오기
  ...
  return (
    <PageProvider initialTotalPages={totalPages}>
      <ViewerSettingsProvider>
        <MainLayout BookID={BookID} bookContent={bookContent} isLoading={isLoading} BookTitle={bookTitle!} BookChapters={bookChapters!}/>
      </ViewerSettingsProvider>
    </PageProvider>
  );
}
...
/** ✅ 메인 레이아웃 */
function MainLayout({ BookID, bookContent, isLoading, BookTitle, BookChapters }: { BookID: string; bookContent: string; isLoading: boolean, BookTitle : string, BookChapters : Chapter[] }) {
  usePageControls();
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 pt-14 h-[calc(100vh-56px)] flex flex-col overflow-hidden">
        ...
        {isLoading ? (
          ...
          <Reader textData={bookContent} bookChapters={BookChapters}/> // ✅ Reader 내부에서도 높이 유지되도록 설정 필요
        )}
      </div>
