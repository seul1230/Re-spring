...
// 전역 PanelContext에서 현재 열린 패널의 ID(currentOpenPanel)를 가져옵니다.
 ...
 *
 * 이 훅은 전역 PanelContext의 currentOpenPanel 값을 확인하여,
 * 만약 어떤 패널이 열려 있다면 (currentOpenPanel !== null)
 * 페이지 이동 및 툴바 토글 이벤트를 무시하도록 합니다.
 */
export function usePageControls() {
  // PageContext에서 현재 페이지 상태를 변경하는 함수를 가져옵니다.
  const { setCurrentPage } = usePageContext();
  // PanelContext에서 현재 열린 패널의 ID를 가져옵니다.
   ...
   * - 단, 전역 패널이 열려 있다면 (currentOpenPanel !== null) 이벤트를 무시합니다.
   */
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // 만약 현재 열린 패널이 있다면, 페이지 이동 이벤트를 무시합니다.
  ...
  }, [currentOpenPanel, setCurrentPage]);
   ...
   * - 단, 전역 패널이 열려 있다면 (currentOpenPanel !== null) 터치 이벤트를 무시합니다.
   */
  useEffect(() => {
    ...
    const handleTouchStart = (event: TouchEvent) => {
      // 만약 현재 열린 패널이 있다면 터치 이벤트를 무시합니다.
      if (currentOpenPanel !== null) return;
