...
const NotificationPage = () => {
  ...
  const toggleReadStatus = (id: number) => {
    setNotifications((prevNotifications) => prevNotifications.map((notif) => (notif.id === id ? { ...notif, read: !notif.read } : notif)));
  };

  const toggleAllReadStatus = () => {
    const allRead = notifications.every((notif) => notif.read);
    setNotifications((prevNotifications) => prevNotifications.map((notif) => ({ ...notif, read: !allRead })));
  };
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-grow min-w-0">
                ...
                <div className="flex justify-between items-center mt-2">
                  <span className="text-xs text-gray-400">{formatDistanceToNow(new Date(notif.createdAt), { addSuffix: true, locale: ko })}</span>
                  <Button
                    variant="outline"
                    size="sm"
