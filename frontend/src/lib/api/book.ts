...
export const getAllBooksAutocomplete = async (query : string) : Promise<BookAutoComplete[]> => {
    ...
    try {
        ...
        throw new Error(error.response?.data?.message || 'getAllBooksAutocomplete 함수 API 호출에서 오류가 발생했습니다.')
    }
}
...
export const getAllBooksComments = async () : Promise<Comment[]> => {
    try {
        ...
        throw new Error(error.response?.data?.message || 'getAllBooksComments 함수 API 호출에서 오류가 발생했습니다.');
    }
}
